...
def _kg_price(hit: dict, ref: dict | None) -> str | None:
    ...
    r = ref or {}
    val  = r.get('unitsOfMeasureOfferPrice') or r.get('unitsOfMeasurePrice') or hit.get('unitOfMeasurePrice')
    unit = r.get('unitsOfMeasurePriceUnit') or hit.get('unitOfMeasurePriceUnits')
    if val and unit:
...
def save_to_supabase(rows: list[dict]):
    ...
    if not rows:
        ...
        return
    client = get_client()
    if not shrink_guard_ok(client, BUTIK, len(rows)):
        return
