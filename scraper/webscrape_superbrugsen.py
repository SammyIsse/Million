...
def parse_netto_vaegt(desc):
    # Match weight/volume patterns like "535-582 g", "1 liter", "600 g", "320-340 g", "33 - 75 cl/6 x 33 cl"
...
def parse_kg_price(desc):
    # Match kg-price like "Kg-pris maks. 18,69" or "Literpris 10,00"
...
def scrape_superbrugsen():
    ...
    try:
        ...
        # Accept cookies if banner appears (just relying on time to bypass or injecting JS to hide if necessary)
        # But wait, we can just scroll the page as is
...
def main():
    ...
    filename = os.path.join(_ROOT_DIR, 'Xlsx filer', 'SuperBrugsen_produkter.xlsx')
    wb.save(filename)
