...
def fetch_active_catalogs() -> list[dict]:
    r = requests.get(
        ...
        params={"dealer_id": LIDL_DEALER_ID, "limit": 20},
        timeout=15,
    )
    r.raise_for_status()
    return r.json()
...
def fetch_lidl_tilbud() -> list[dict]:
    ...
    print(f"  Fandt {len(catalogs)} aktive Lidl-kataloger")
    ...
    assert_catalogs_healthy("Lidl", len(catalogs), _tomme_kataloger)

    attach_billede_hashes(rows)
    print(f"  OK: {len(rows)} Lidl tilbud hentet fra Tjek API")
    return rows
...
def main():
    print("Starter Lidl scraper (Tjek API)...")
