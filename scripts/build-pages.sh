...
# DEBUG_OBSERVABILITY_STAGING: engangs-undtagelse til at diagnosticere
# staging-ustabiligheden 2026-07-25 (roegtesten fejler 30-50/60 under
# samtidig trafik, ingen logs at forklare hvorfor). KUN til effekt naar
# DEPLOY_ENV=staging OG varen eksplicit er "1" - produktion kan aldrig
# roeres af den, og standardadfaerden (varen usat) er uaendret "false"
# begge steder. Saet ALDRIG denne var i en committed workflow-fil uden en
# tilhoerende revert-plan; fjern denne hele blok igen naar fejlen er
# diagnosticeret.
if [ "$DEPLOY_ENV" = "staging" ] && [ "${DEBUG_OBSERVABILITY_STAGING:-}" = "1" ]; then
  OBS_ENABLED="true"
else
  OBS_ENABLED="false"
fi
...
#
# OBS_ENABLED sættes ovenfor (kun "true" med DEBUG_OBSERVABILITY_STAGING=1
# på staging) - se kommentaren dér.
[observability]
enabled = ${OBS_ENABLED}
