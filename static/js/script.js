 ...
 * kan redigere - så indholdet er ikke betroet markup. Kilderne bruger dog
 * <b> til den lovpligtige allergen-fremhævning og <br> til linjeskift, så vi
 * kan ikke bare escape alt. Derfor: escape ALT, og gendan kun de to tags i
 * deres attributløse form. "<b onclick=...>" bliver til "&lt;b onclick=...&gt;"
 * og matcher ikke mønstrene nedenfor - det forbliver ufarlig tekst.
 */
...
function renderNutritionSection(productId) {
    ...
    _nutritionCache[pid].then(data => {
        ...
        // Escapes: næringsdata kommer fra tredjeparter (bl.a. Open Food Facts,
        // som alle kan redigere), så label/value/ingredienser er ikke betroet
